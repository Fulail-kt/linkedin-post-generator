...
/**
 * A specialized hook for the GeneratePost BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.LinkedInPostResponse
 * - **Streaming Partial:** LinkedInPostResponse
 * - **Streaming Final:** types.LinkedInPostResponse
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useGeneratePost({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useGeneratePost({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useGeneratePost(props: HookInput<'GeneratePost', { stream: false }>): HookOutput<'GeneratePost', { stream: false }>
export function useGeneratePost(props?: HookInput<'GeneratePost', { stream?: true }>): HookOutput<'GeneratePost', { stream: true }>
export function useGeneratePost(
  props: HookInput<'GeneratePost', { stream?: boolean }> = {},
): HookOutput<'GeneratePost', { stream: true }> | HookOutput<'GeneratePost', { stream: false }> {
  let action: ServerAction = Actions.GeneratePost;
  if (isStreamingProps(props)) {
    action = StreamingActions.GeneratePost;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'GeneratePost', { stream: false }>)
  }
}